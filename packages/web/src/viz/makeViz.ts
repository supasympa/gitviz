...
export const makeViz = (chartNode: any, gitLogData: any) => {
    ...
    d3.select(chartNode).select('.spinner-border').remove();
    ...
    const calcFillColor = function(d: any) {
        ...
        const range = (d3.range(0, 10) as ReadonlyArray<
            unknown
        >) as ReadonlyArray<string>; //WTF Typescript!?
        if (!d.parent) {
